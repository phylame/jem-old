...
public class Worker {
    ...
    public Map<String, Object> getMakeArguments(Component parent, String format) {
        Map<String, Object> kw = MacFactory.getArguments(parent,
                app.getText("Dialog.MacProvider.Title", format.toUpperCase()), format);
        addPmabInfo(kw);
        return kw;
    }

    private void addPmabInfo(Map<String, Object> kw) {
        if (kw == null) {
            return;
        }

        String info = "generated by " + app.getText("App.Name") + " (" + Application.VERSION +
                ")";
        ...
        if (! kw.containsKey(PmabMaker.KEY_META_DATA)) {
            ...
            metaInfo.put("generator", app.getText("App.Name") + " v" + Application.VERSION);
    ...
    public boolean saveBook(Component parent, String title, Task task) {
        ...
        Map<String, Object> kw = getMakeArguments(parent, Jem.PMAB_FORMAT);
        if (kw == null) {
            return false;
        }

        try {
            Jem.writeBook(task.getBook(), path, Jem.PMAB_FORMAT, kw);
        } catch (IOException | JemException e) {
